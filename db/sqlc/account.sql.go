...
const updateAccountForUpdate = `-- name: UpdateAccountForUpdate :one
UPDATE accounts SET balance = $2
WHERE id = $1
RETURNING id, owner, balance, currency, "createdAt"
`

type UpdateAccountForUpdateParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) UpdateAccountForUpdate(ctx context.Context, arg UpdateAccountForUpdateParams) (Account, error) {
	row := q.db.QueryRow(ctx, updateAccountForUpdate, arg.ID, arg.Balance)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
